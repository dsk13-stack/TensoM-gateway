...
class UiMainWindow(ui.MainWindow):
    ...
    импрортируются из файла ui вместе с классом.
                ...
                try:
                    ...
                    for i in range(row_count):
                        ...
                        if read_buffer == b"" or len(read_buffer) < 9 or read_buffer[2] == b"EE":
                            logger.warning("Повторное чтение данных устройства с адресом {addr}".format(addr=addr))
                            serial_connection.write(packet)
                            read_buffer = serial_connection.read_until(expected=b"\xff\xff")

                            if read_buffer == b"":
                                logger.warning("Устройство с адресом {addr} недоступно".format(addr=addr))
                                self.change_cell(table, i, data_colum, "-32768.0")
                                self.change_cell(table, i, status_column, "Прибор недоступен", QtGui.QColor(255, 0, 0))
                            elif len(read_buffer) < 9 or read_buffer[2] == b"EE":
                                logger.warning("Устройство с адресом {addr} в ошибке".format(addr=addr))
                                self.change_cell(table, i, data_colum, "-32768.0")
                                self.change_cell(table, i, status_column, "Ошибка прибора", QtGui.QColor(255, 0, 0))
                            else:
